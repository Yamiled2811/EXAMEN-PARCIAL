/* app.js - Paso 1 + Paso 2 combinados (fetch, catálogo, routing, detalle, favoritos, carrito) */
...
const els = {
  ...
  // carrito (definimos aquí para que existan antes de attachEvents)
...
function attachEvents(){
  ...
  // carrito: el botón puede no existir durante pruebas; proteger
  if(els.cartBtn){
    els.cartBtn.addEventListener('click', openCart);
  }
  // Hash routing handled elsewhere
}

/* --- Storage helpers (favoritos, carrito) --- */
const storage = {
...
function addToCart(item){ // item: {id, qty, max}
...
function closeDetail(){
  // remove hash without reloading (go back to root)
...
function buildDetailHTML(ev){
  // usar la primera imagen como poster grande y las imágenes siguientes como miniaturas
  const mainSrc = ev.images?.[0] || '';
  const thumbs = (ev.images || []).slice(1, 6); // tomar varias miniaturas (pos 1..5)
  const thumbsHtml = thumbs.map((src, idx) => {
    const realIdx = idx + 1; // índice real dentro de ev.images
    const artistLabel = ev.artists?.[idx] || '';
    return `
      <figure class="thumb">
        <img src="${src}" data-idx="${realIdx}" alt="${ev.title} imagen ${realIdx+1}" ${idx===0? 'aria-selected="true"': ''}>
...
function handleHash(){
  ...
  if(h.startsWith('#/event/')){
    ...
    // si no hay hash de detalle, cerrar modal si abierto
...
/* --- Eventos del detalle (thumbs, fav, share, add to cart) --- */
...
async function init(){
  ...
  // Si al cargar la URL ya tiene hash de detalle, handleHash() lo mostrará por el listener 'load'
}
init();
...
function renderCheckout(total){
  ...
  // attach close handler for checkout dialog
